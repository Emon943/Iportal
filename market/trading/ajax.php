...
elseif($function=='createPortfolio'){
 ...
 $sql = "INSERT INTO tr_profile (user_id,portfolio_name,portfolio_amount,comission_rate,creation_date) VALUES('$userid', '$portfolio_name', '$portfolio_amount', '$comission_rate', '$creation_date')";
 $sql2 ="INSERT INTO tr_investment (profile_id, user_id, amount) VALUES (LAST_INSERT_ID(), '$userid', '$portfolio_amount')";
...
elseif($function=='getBalance'){
	...
	$query = mysql_query("SELECT portfolio_amount FROM tr_profile WHERE ID = $portfolio_id AND user_id = $userid");
	while($info = mysql_fetch_array($query)) {
		$data = array(
...
elseif($function=='addFund'){
	...
	$userid 	  = $_POST['user_id'];
	$fund 		  = $_POST['fund'];
$query 	= mysql_query("UPDATE tr_profile SET portfolio_amount = portfolio_amount + $fund WHERE ID = $portfolio_id and user_id= $userid");
$query2 = mysql_query("INSERT INTO tr_investment (`profile_id`, `user_id`, `amount`) VALUES ('$portfolio_id', '$userid', '$fund')");
...
elseif($function=='withdrawFund'){
	...
	$fund 		  = $_POST['fund'];
$query 	= mysql_query("UPDATE tr_profile SET portfolio_amount = portfolio_amount - $fund WHERE ID = $portfolio_id and user_id= $userid");
$query2 = mysql_query("INSERT INTO tr_withdraw (`profile_id`, `user_id`, `amount`) VALUES ('$portfolio_id', '$userid', '$fund')");
...
$query 	= mysql_query("SELECT portfolio_name, comission_rate FROM tr_profile WHERE ID = '$portfolio_id' AND user_id = '$userid'");
...
$query 	= mysql_query("UPDATE tr_profile SET portfolio_name = '$portfolio_name', comission_rate = '$com'  WHERE ID = $portfolio_id and user_id= $userid");
...
$query 	= mysql_query("DELETE FROM tr_profile
...
elseif($function=='sellexecute'){
	...
	$query  = mysql_query("UPDATE tr_portfolio SET sell_share_amount = sell_share_amount + $sell_share_amount, sell_price = $sell_price, total_sell_price = $total_sell_price,  `sell_date` = CAST('$sell_date' AS DATE), sell_commision = $sell_com WHERE portfolio_ID = '$portfolio_ID' AND company_id = '$comp_id';");

	$query = mysql_query("INSERT INTO tr_accounts (`portfolio_id`,`profile_id`,`user_id`,`company_id`,`sell_price`,`sell_amount`,`sell_commision`,`sell_date`) VALUES ('$portfolio_ID','$profile_id','$user_id','$comp_id','$sell_price','$sell_share_amount','$sell_com',CAST('$sell_date' AS DATE))");

	$query = mysql_query("UPDATE tr_profile SET portfolio_amount = portfolio_amount + $total_sell_price WHERE user_id = $user_id AND ID = $profile_id");
	if($query) {
...
elseif($function=='stay'){
	...
	stayOrder($userid,$profile_id,$order_type,$company_id,$amount,$buy_price,$buy_date,$order_val,$expiry_date);
}
?>